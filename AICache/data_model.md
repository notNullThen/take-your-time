...
The application notably **does not use years**. It operates on a perpetual annual cycle to simplify inputs.
```typescript
...
## 3. Business Logic (`calculations.ts`)
The core algorithms of the app live here.

**Overwork Expiration Policy**:
Because years are excluded from the data model, expiration must calculate "months elapsed" cyclically using modulo arithmetic:
```typescript
...
- **Overwork** (positive balance) is discarded from the `Total Net Balance` if `monthsDiff > settings.expirationMonths`.
